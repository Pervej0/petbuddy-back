...
export const deleteAdoptionRequests: RequestHandler = asyncCatch(
  async (req: any, res) => {
    const result = await deleteAdoptionRequestsDB(
      req.user,
      req.params.requestId
    );
    sendResponse(res, {
      statusCode: StatusCodes.OK,
      message: "Adoption requests deleted successfully",
      data: result,
    });
  }
);
